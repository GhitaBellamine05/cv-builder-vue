...
import data_uz from "./data/data_uz.json"
...
export const useSettingStore = defineStore("settings", {
    ...
    actions: {
        cv_change_languages(language) {
            switch (language) {
                case "english":
                    ...
                    break;
                case "uzbek":
                    localStorage.setItem("cv_data", JSON.stringify(data_uz))
                    this.cv_data = JSON.parse(localStorage.getItem("cv_data"))
                    break;
                case "russia":
                    localStorage.setItem("cv_data", JSON.stringify(data_ru))
                    this.cv_data = JSON.parse(localStorage.getItem("cv_data"))
                    break;
            }
        },
        ...
        cv_remove() {
            ...
            let data = JSON.parse(localStorage.getItem("cv_data")).personal_details.first_name
            if (conf) {
        ...
        cv_download() {
            ...
            link.href = url;
            link.download = "CV_" + this.cv_data.personal_details.first_name + "_" + this.cv_data.personal_details.last_name + "_" + this.cv_data.settings.cv + "_" + new Date().getFullYear();
